...
```cmd
rmdir /s /q proj-04 2>nul & xcopy baseproj proj-04 /e /i /q & cd proj-04 & tree /f
```
...
easier to refer to and view them relative to (within) our `proj-04/` project
...
```log
node ➜ /workspaces/cap-tour-hands-on/proj-04/ (main) $ cds repl
Welcome to cds repl v10.0.3
> cds.home
/workspaces/cap-tour-hands-on/proj-04/node_modules/@sap/cds
> cds.root
/workspaces/cap-tour-hands-on/proj-04
>
```
...
```log
Wrote to [...]/proj-04/flags/package.json:
...
section in `proj-04`'s `package.json`:
...
👉 Now start up the main service in `proj-04/` like we did before:
...
👉 So let's do that now (in a separate terminal, in `proj-04/` of course):
